<template>
  <swiper v-if="articleList.length > 0" class="swiper-container" :modules="modules" :loop="true" :slides-per-view="1"
    ...
    <swiper-slide v-for="article in articleList" :key="article.no">
      <div class="slide-content" :style="articleCover(article.thumbnail)">
        <router-link :to="`/article/${article.no}`" class="slide-title">{{ article.title }}</router-link>
