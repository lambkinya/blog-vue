  ...
  <div v-if="!$common.isEmpty(article)">
    ...
    <div class="article-head my-animation-slide-top">
      ...
      <div class="article-info-news" @click="weiYanDialogVisible = true"
           v-if="!$common.isEmpty($store.state.loginCoder) && $store.state.loginCoder.no === article.coderNo">
      ...
      <div class="article-container my-animation-slide-bottom">
        ...
        <div class="article-sort">
          <span
              ...
              article.categoryInfo.name + " ▶ " + article.tagInfo.name
            }}</span>
        </div>
        <!-- 作者信息 -->
        <blockquote>
...
export default {
  ...
  methods: {
    ...
    getNews() {
      const source = this.article.no
      this.$http.post(this.$constant.baseURL + "/weiYan/list/" + source + "/3", {
        current: 1,
        size: 9999,
      })
          .then((res) => {
