...
<template>
  <el-row class="sharelistBox">
    ...
    <el-col :span="24" class="s-item tcommonBox" v-for="(item,index) in articleList" :key="'article'+index">
      ...
      <header>
        <h1>
          <a :href="'#/DetailArticle?ano='+item.no" target="_blank">
            {{item.title}}
          </a>
        </h1>
        <h2>
        ...
        <div class="ui label">
          <a :href="'#/Share?categoryNo='+item.no">{{item.categoryName}}</a>
        </div>
      </header>
      ...
      <div class="viewdetail">
        <a class="tcolors-bg" :href="'#/DetailArticle?ano='+item.no" target="_blank">
          阅读全文>>
        </a>
      </div>
    ...
    methods: { //事件处理器
      ...
      getList () {
        articleList(this.queryParams).then((response) => {
          this.articleList = this.articleList.concat(response.records)
      ...
      routeChange: function () {
        var that = this;
        //获取传参的categoryNo
        this.queryParams.categoryNo = (that.$route.query.categoryNo == undefined ? "" : that.$route.query.categoryNo);
        console.log("分页查询所携带的分类ID", this.queryParams.categoryNo);
