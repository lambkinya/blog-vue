<template>
  <n-modal class="bg" v-model:show="dialogVisible" preset="dialog" :show-icon="false" transform-origin="center"
    :block-scroll="false">
    <n-input class="mt-11" v-model:value="loginForm.account" placeholder="账号/邮箱" @keyup.enter="handlelogin"></n-input>
...
<script setup lang="ts">
  ...
  const handlelogin = () => {
    ...
    // if (!reg.test(loginForm.value.account)) {
    //   window.$message?.warning("账号或邮箱格式不正确");
    //   return;
    // }
    if (loginForm.value.account.trim().length == 0) {
      window.$message?.warning("账号或邮箱不能为空");
      return;
    }
